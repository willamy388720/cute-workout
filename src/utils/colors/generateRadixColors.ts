...
type RadixColorsProps = {
  accent: string;
};

export const generateRadixColors = ({ accent }: RadixColorsProps) => {
  const allScales = lightColors;
  const grayScales = lightGrayColors;
  const backgroundColor = new Color("#FFFFFF").to("oklch");

  const grayBaseColor = new Color("#8B8D98").to("oklch");
  ...
  const accentSurfaceHex = getAlphaColorSrgb(
    accentScaleHex[1],
    backgroundHex,
    0.8
  );

  const accentSurfaceWideGamutString = getAlphaColorP3(
